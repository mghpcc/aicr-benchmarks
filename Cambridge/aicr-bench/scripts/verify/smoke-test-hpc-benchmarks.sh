...
image="${1:-${HPCBENCH_IMAGE:-${AICR_APPTAINER_IMAGE_DIR}/hpc-benchmarks-26.02.sif}}"
run_id="${SETUP_RUN_ID:-$(aicr_next_setup_run_id "$cluster" "$AICR_CHECK_HPC_BENCHMARKS_SMOKE")}" 
raw_rel="$(aicr_setup_raw_run_dir "$cluster" "$AICR_CHECK_HPC_BENCHMARKS_SMOKE" "$run_id")"
parsed_rel="$(aicr_setup_parsed_run_dir "$cluster" "$AICR_CHECK_HPC_BENCHMARKS_SMOKE" "$run_id")"
...
summary_rel="${raw_rel}/canonical/hpc-benchmarks-smoke-summary.txt"; wrapper_out_rel="${raw_rel}/wrapper/slurm-${SLURM_JOB_ID:-manual}.out"; wrapper_err_rel="${raw_rel}/wrapper/slurm-${SLURM_JOB_ID:-manual}.err"; : > "${AICR_BMARK_DIR}/${wrapper_out_rel}"; : > "${AICR_BMARK_DIR}/${wrapper_err_rel}"
...
if [[ "$status" == "passed" ]] && command -v apptainer >/dev/null 2>&1; then apptainer exec ${AICR_APPTAINER_COMMON_OPTS} --nv "$image" bash -lc 'echo hpc-benchmarks-smoke' > "${AICR_BMARK_DIR}/${wrapper_out_rel}" 2> "${AICR_BMARK_DIR}/${wrapper_err_rel}" || status="failed"; else echo "apptainer not available or image missing" > "${AICR_BMARK_DIR}/${wrapper_err_rel}"; fi
...
PY
record_rel="$(aicr_setup_record_path "$cluster" "$AICR_CHECK_HPC_BENCHMARKS_SMOKE" "$run_id")"
