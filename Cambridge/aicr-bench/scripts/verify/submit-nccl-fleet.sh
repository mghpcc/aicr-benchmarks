...
usage() {
  cat >&2 <<'EOF'
Usage:
  scripts/verify/submit-nccl-fleet.sh --scope local --cluster <b200|rtxpro6000> [options]
  scripts/verify/submit-nccl-fleet.sh --scope rdma --cluster <b200|rtxpro6000> --nodes-per-job <n> [options]
  scripts/verify/submit-nccl-fleet.sh --scope survey --cluster <b200|rtxpro6000> [options]

Dry-run-first submitter for the fully instrumented NCCL suite.
Survey scope submits one survey size at a time in apply mode: all 1-node jobs finish
before 2-node jobs, 2-node before 4-node, and so on. The dashboard renders
after the full selected survey ladder completes.

Options:
  --apply                    Submit jobs with sbatch
  --scope <local|rdma|survey> Suite scope
  ...
  --nodes-per-job <n>        RDMA group size
  --survey-sizes <list>      Survey node counts, comma/space-separated
  ...
  --survey-stagger-seconds <n>  Additional delay after one survey size finishes before the next starts (default: 0)
  --round-stagger-seconds <n>   Delay between repeat rounds (default: 0)
  --allow-nonstandard-node-count
                            Permit RDMA/survey node counts outside the
                            standard B200 1/2/4/8/16 and RTX 1/2/4/8 ladders
...
default_survey_sizes_for_cluster() {
  case "$1" in
    b200) printf '1 2 4 8 16\n' ;;
    rtxpro6000) printf '1 2 4 8\n' ;;
  esac
}

validate_survey_size_for_cluster() {
  local cluster_name="$1"
  local survey_size="$2"
  if [[ "${allow_nonstandard_node_count:-0}" == "1" ]]; then
    [[ "${survey_size}" =~ ^[0-9]+$ && "${survey_size}" -ge 1 ]] || {
      echo "ERROR: nonstandard survey size must be positive: ${survey_size}" >&2
      exit 2
    }
    return 0
  fi
  case "${cluster_name}:${survey_size}" in
    b200:1|b200:2|b200:4|b200:8|b200:16|rtxpro6000:1|rtxpro6000:2|rtxpro6000:4|rtxpro6000:8) ;;
    *) echo "ERROR: invalid survey size ${survey_size} for ${cluster_name}" >&2; exit 2 ;;
  esac
}
...
write_survey_manifest() {
  ...
  local idle_json skipped_json gpu_preflight_excluded_json suite_ops
  ...
  suite_ops="${NCCL_SUITE_OPS:-allreduce,allgather,reduce_scatter,alltoall}"

  aicr_python - \
    "$path" "$scope" "$cluster" "$partition" "$discovered_at_utc" "$mode" "$wait_result" \
    "$markdown_report" "$submit_stagger_seconds" "$survey_stagger_seconds" "$round_stagger_seconds" "$repeat_count" \
    ...
    "$profile" "$survey_sizes_text" "$idle_json" "$selected_groups_file" "$submitted_jobs_file" \
    "$skipped_json" "$suite_ops" <<'PY'
import json
import sys
from pathlib import Path
...
selected_survey_sizes = [int(item) for item in survey_sizes_text.split() if item]
suite_ops = [item for item in suite_ops_text.replace(",", " ").split() if item]

selected_groups = []
for line in Path(selected_groups_path).read_text(encoding="utf-8").splitlines():
    if not line.strip():
        continue
    survey_size_text, group = line.split("|", 1)
    nodes = [item for item in group.split(",") if item]
    selected_groups.append({
        "survey_size": int(survey_size_text),
        "scale": int(survey_size_text),
        "group": group,
        "nodes": nodes,
...
if Path(submitted_jobs_path).exists():
    for line in Path(submitted_jobs_path).read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        round_text, survey_size_text, group, job_id = line.split("|", 3)
        nodes = [item for item in group.split(",") if item]
        item = {
...
obj = {
    ...
    "selected_survey_sizes": selected_survey_sizes,
    "selected_scales": sorted({item["scale"] for item in selected_groups}),
    ...
    "survey_submission_policy": "sequential-wait",
    ...
    "survey_stagger_seconds": int(survey_stagger_seconds),
...
wait_for_jobs() {
  ...
  if command -v sacct >/dev/null 2>&1; then
    bad_jobs="$(
      sacct -n -P -j "$jobs_csv" --format=JobID,State,ExitCode 2>/dev/null \
        | awk -F'|' 'index($1, ".") == 0 && index($1, "_") == 0 && !($2 == "COMPLETED" && $3 == "0:0") {print}'
    )"
    if [[ -n "${bad_jobs}" ]]; then
      echo "ERROR: one or more NCCL suite jobs did not complete cleanly:" >&2
      echo "${bad_jobs}" | sed 's/^/  /' >&2
      return 1
    fi
  fi
}
...
survey_stagger_seconds=0
round_stagger_seconds=0
apply=0
wait_for_completion=1
render=1
allow_nonstandard_node_count=0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --nodes-per-job)
      ...
      ;;
    --survey-sizes)
      survey_sizes="${2:?missing value for --survey-sizes}"
      shift 2
      ;;
    --partition)
    ...
    --submit-stagger-seconds)
      ...
      shift 2
      ;;
    --survey-stagger-seconds)
      survey_stagger_seconds="${2:?missing value for --survey-stagger-seconds}"
      shift 2
      ;;
    --round-stagger-seconds)
      ...
      ;;
    --allow-nonstandard-node-count)
      allow_nonstandard_node_count=1
      shift
      ;;
    --no-wait)
...
case "${scope}" in local|rdma|survey) ;; *) echo "ERROR: --scope must be local, rdma, or survey" >&2; exit 2 ;; esac
case "${profile}" in small|medium|large) ;; *) echo "ERROR: --profile must be small, medium, or large" >&2; exit 2 ;; esac
...
[[ "${survey_stagger_seconds}" =~ ^[0-9]+$ ]] || { echo "ERROR: --survey-stagger-seconds must be non-negative" >&2; exit 2; }
...
case "${scope}:${cluster}" in
  ...
  survey:b200) sbatch_path="slurm/verify/b200-nccl-suite-survey.sbatch" ;;
  survey:rtxpro6000) sbatch_path="slurm/verify/rtxpro6000-nccl-suite-survey.sbatch" ;;
...
survey_sizes_text=""
if [[ "${scope}" == "survey" ]]; then
  if [[ -n "${nodes_per_job}" && -n "${survey_sizes}" ]]; then
    echo "ERROR: use either --nodes-per-job or --survey-sizes with --scope survey, not both" >&2
    exit 2
  fi
  survey_sizes="${survey_sizes:-${nodes_per_job:-$(default_survey_sizes_for_cluster "${cluster}")}}"
  for survey_size in ${survey_sizes//,/ }; do
    [[ -n "${survey_size}" ]] || continue
    [[ "${survey_size}" =~ ^[0-9]+$ ]] || { echo "ERROR: survey size must be numeric: ${survey_size}" >&2; exit 2; }
    validate_survey_size_for_cluster "${cluster}" "${survey_size}"
    survey_sizes_text="${survey_sizes_text} ${survey_size}"
    survey_groups_file="${tmpdir}/survey-${survey_size}-groups.txt"
    build_groups "${idle_nodes_file}" "${survey_groups_file}" "${survey_size}"
    if [[ ! -s "${survey_groups_file}" ]]; then
      if [[ "${gpu_preflight_filter}" == "1" ]]; then
        echo "ERROR: fewer than ${survey_size} GPU-preflight-passed exactly-idle ${cluster} nodes are available for survey size ${survey_size}" >&2
      else
        echo "ERROR: fewer than ${survey_size} exactly-idle ${cluster} nodes are available for survey size ${survey_size}" >&2
      fi
      exit 1
    fi
    while IFS= read -r group; do
      [[ -n "${group}" ]] || continue
      printf '%s|%s\n' "${survey_size}" "${group}" >>"${selected_groups_file}"
    done <"${survey_groups_file}"
  done
  survey_sizes_text="${survey_sizes_text# }"
elif [[ "${scope}" == "rdma" ]]; then
  nodes_per_job="${nodes_per_job:-2}"
  if [[ "${allow_nonstandard_node_count}" == "1" ]]; then
    [[ "${nodes_per_job}" =~ ^[0-9]+$ && "${nodes_per_job}" -ge 2 ]] || {
      echo "ERROR: nonstandard --nodes-per-job must be an integer >= 2: ${nodes_per_job}" >&2
      exit 2
    }
  else
...
echo "allow_nonstandard_node_count=$([[ "${allow_nonstandard_node_count}" == "1" ]] && echo enabled || echo disabled)"
if [[ "${allow_nonstandard_node_count}" == "1" ]]; then
  echo "WARNING: nonstandard node counts are outside the standard NCCL ladders."
fi
echo "submit_stagger_seconds=${submit_stagger_seconds}"
if [[ "${scope}" == "survey" ]]; then
  echo "survey_stagger_seconds=${survey_stagger_seconds}"
fi
echo "round_stagger_seconds=${round_stagger_seconds}"
if [[ "${scope}" == "rdma" ]]; then
  echo "nodes_per_job=${nodes_per_job}"
elif [[ "${scope}" == "survey" ]]; then
  echo "survey_sizes=${survey_sizes_text}"
fi
echo
if [[ "${gpu_preflight_filter}" == "1" ]]; then
...
if [[ "${apply}" == "1" && "${scope}" == "survey" && "${wait_for_completion}" != "1" ]]; then
  survey_size_count="$(wc -w <<<"${survey_sizes_text}" | tr -d ' ')"
  if [[ "${survey_size_count}" -gt 1 ]]; then
    echo "ERROR: --scope survey with multiple survey sizes requires waiting between survey sizes; omit --no-wait" >&2
    exit 2
  fi
fi
...
if [[ "${apply}" == "0" ]]; then
  ...
  for round in $(seq 1 "${repeat_count}"); do
    ...
    previous_survey_size=""
    while IFS='|' read -r survey_size group; do
      [[ -n "${survey_size}" && -n "${group}" ]] || continue
      if [[ "${scope}" == "survey" && -n "${previous_survey_size}" && "${survey_size}" != "${previous_survey_size}" ]]; then
        echo "  # wait for all ${previous_survey_size}n jobs before submitting ${survey_size}n jobs when --apply is used"
        if [[ "${survey_stagger_seconds}" != "0" ]]; then
          echo "  # sleep ${survey_stagger_seconds} after ${previous_survey_size}n jobs finish before ${survey_size}n survey submissions"
        fi
      fi
      if [[ "${scope}" == "local" ]]; then
        ...
        ntasks=$(( survey_size * 8 ))
        echo "  sbatch --parsable --nodes=${survey_size} --ntasks=${ntasks} --ntasks-per-node=8 --cpus-per-task=16 --nodelist=${group} --time=${time_limit} ${sbatch_path} ${base_args[*]} --nodes-per-job ${survey_size}"
      fi
      previous_survey_size="${survey_size}"
...
wait_result="not-run"
if [[ "${scope}" == "survey" ]]; then
  manifest_id="$(date -u +%H%M%SZ)-nccl-suite-survey-${cluster}"
  manifest_rel="results/reports/${date_utc}/nccl-suite-survey/${manifest_id}.json"
  manifest_abs="${AICR_BMARK_DIR}/${manifest_rel}"
elif [[ "${scope}" == "local" ]]; then
  manifest_id="$(date -u +%H%M%SZ)-nccl-suite-local-${cluster}"
  manifest_rel="results/reports/${date_utc}/nccl-suite-local/${manifest_id}.json"
  manifest_abs="${AICR_BMARK_DIR}/${manifest_rel}"
else
  manifest_id="$(date -u +%H%M%SZ)-nccl-suite-rdma-${cluster}-${nodes_per_job}n"
  manifest_rel="results/reports/${date_utc}/nccl-suite-rdma/${manifest_id}.json"
  manifest_abs="${AICR_BMARK_DIR}/${manifest_rel}"
fi
for round in $(seq 1 "${repeat_count}"); do
  echo "Round ${round}/${repeat_count}"
  round_jobs_file="${tmpdir}/round-${round}-jobs.txt"
  survey_jobs_file="${tmpdir}/round-${round}-survey-jobs.txt"
  : >"${round_jobs_file}"
  : >"${survey_jobs_file}"
  ...
  while IFS='|' read -r survey_size group; do
    [[ -n "${survey_size}" && -n "${group}" ]] || continue
    if [[ "${scope}" == "survey" && "${wait_for_completion}" == "1" && -n "${current_survey_size}" && "${survey_size}" != "${current_survey_size}" ]]; then
      write_survey_manifest "${manifest_abs}" "apply" "submitted" ""
      echo "Wrote recoverable pre-wait manifest ${manifest_rel}"
      wait_for_jobs "$(paste -sd, - <"${survey_jobs_file}")"
      wait_result="completed"
      : >"${survey_jobs_file}"
      if [[ "${survey_stagger_seconds}" != "0" ]]; then
        echo "Sleeping ${survey_stagger_seconds}s before survey size ${survey_size} submissions"
        sleep "${survey_stagger_seconds}"
      fi
    fi
    ...
    if [[ "${scope}" == "local" ]]; then
      ...
      job_id="$(
        sbatch --parsable \
          ...
          "${sbatch_path}" \
          "${base_args[@]}" \
          --nodes-per-job "${survey_size}"
      )"
    fi
    job_id="${job_id%%;*}"
    [[ -n "${job_id}" ]] || aicr_die "sbatch did not return a job ID for ${group}"
    printf '%s|%s|%s|%s\n' "${round}" "${survey_size}" "${group}" "${job_id}" >>"${submitted_jobs_file}"
    printf '%s\n' "${job_id}" >>"${round_jobs_file}"
    printf '%s\n' "${job_id}" >>"${survey_jobs_file}"
    echo "Submitted round ${round} survey size ${survey_size} ${group} as job ${job_id}"
    submitted_this_round=$((submitted_this_round + 1))
    previous_survey_size="${survey_size}"
    current_survey_size="${survey_size}"
  done <"${selected_groups_file}"
  if [[ "${scope}" == "survey" && "${wait_for_completion}" == "1" ]]; then
    write_survey_manifest "${manifest_abs}" "apply" "submitted" ""
    echo "Wrote recoverable pre-wait manifest ${manifest_rel}"
    wait_for_jobs "$(paste -sd, - <"${survey_jobs_file}")"
    wait_result="completed"
...
if [[ "${render}" == "1" && "${wait_for_completion}" == "1" ]]; then
  if [[ "${scope}" == "local" ]]; then
    ...
    write_survey_manifest "${manifest_abs}" "apply" "${wait_result}" "${report_rel}"
    scripts/report/render-nccl-suite-report.py --date "${date_utc}" --cluster "${cluster}" --scope local --fleet-manifest "${manifest_abs}" --output "${report_rel}"
    echo "Wrote ${manifest_rel}"
  elif [[ "${scope}" == "rdma" ]]; then
    report_rel="results/reports/${date_utc}/nccl-suite-rdma-${cluster}-${nodes_per_job}n.md"
    write_survey_manifest "${manifest_abs}" "apply" "${wait_result}" "${report_rel}"
    scripts/report/render-nccl-suite-report.py --date "${date_utc}" --cluster "${cluster}" --scope rdma --nodes-per-job "${nodes_per_job}" --repeat-aggregation "${repeat_aggregation}" --fleet-manifest "${manifest_abs}" --output "${report_rel}"
    echo "Wrote ${manifest_rel}"
  else
    report_rel="results/reports/${date_utc}/nccl-suite-survey-${cluster}.md"
    write_survey_manifest "${manifest_abs}" "apply" "${wait_result}" "${report_rel}"
    scripts/report/render-nccl-suite-report.py --date "${date_utc}" --cluster "${cluster}" --scope survey --fleet-manifest "${manifest_abs}" --output "${report_rel}"
    echo "Wrote ${manifest_rel}"
  fi
  echo "Wrote ${report_rel}"
elif [[ -n "${manifest_abs}" ]]; then
  write_survey_manifest "${manifest_abs}" "apply" "${wait_result}" ""
  echo "Wrote ${manifest_rel}"
fi
