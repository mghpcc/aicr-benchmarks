...
Default submit stagger is 30 seconds; use 0 only for intentional concurrent filesystem stress.
Use --submit-stagger-seconds benchmark to submit selected jobs as an afterany dependency chain.
Default profile is small. Default repeat count is 1. Default repeat aggregation is standard.
GPU preflight filtering is opt-in and uses latest same-day gpu-topology summaries.
EOF
}
...
obj = {
    ...
    "submit_stagger_policy": submit_stagger_policy,
...
render=1
submit_stagger_policy="numeric"
submit_stagger_seconds=30
benchmark_submit_pacing_seconds=5
...
if [[ "$submit_stagger_seconds" == "benchmark" ]]; then
  submit_stagger_policy="benchmark"
  submit_stagger_seconds=0
elif ! [[ "$submit_stagger_seconds" =~ ^[0-9]+$ ]]; then
...
if [[ "$submit_stagger_policy" == "benchmark" ]]; then
  echo "Submit stagger  : benchmark dependency chain (${benchmark_submit_pacing_seconds}s sbatch pacing)"
else
...
if [[ "$apply" == "0" ]]; then
  ...
  for round in $(seq 1 "$repeat_count"); do
    ...
    while read -r node; do
      [[ -n "$node" ]] || continue
      dependency_flag=""
      if [[ "$submit_stagger_policy" == "benchmark" && -n "$previous_job_label" ]]; then
  ...
  if [[ "$idle_count" != "0" ]]; then
    if [[ "$submit_stagger_policy" == "benchmark" ]]; then
      echo "  # benchmark stagger uses --dependency=afterany:<previous-gds-job-id> after the first job"
      echo "  # sleep ${benchmark_submit_pacing_seconds} between sbatch calls when --apply is used"
    else
...
for round in $(seq 1 "$repeat_count"); do
  ...
  while read -r node; do
    [[ -n "$node" ]] || continue
    dependency_args=()
    if [[ "$submit_stagger_policy" == "benchmark" && -n "$previous_job_id" ]]; then
      sleep "$benchmark_submit_pacing_seconds"
      dependency_args=(--dependency="afterany:${previous_job_id}")
    elif [[ "$submitted_count" != "0" && "$submit_stagger_seconds" != "0" ]]; then
    ...
    if [[ "${#dependency_args[@]}" -gt 0 ]]; then
      echo "Submitted round ${round} ${node} as job ${job_id} afterany:${previous_job_id}"
    else
  ...
  if [[ "$round" != "$repeat_count" && "$wait_for_completion" == "1" && "$round_stagger_seconds" != "0" ]]; then
