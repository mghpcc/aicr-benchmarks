...
#   PARTITION    Slurm partition (default b200-batch)
...
#   sbatch --array=0-15%16 peak_aggregate_read.sh
#
#   # rtx-batch (override partition via sbatch -p; -J keeps stdout files clean):
#   sbatch -p rtx-batch -J vast_peak_read_rtx --array=0-7%8 peak_aggregate_read.sh
#
#   # with env overrides:
#   ARRAY_SIZE=20 NODES_PER=2 sbatch --array=0-19%20 peak_aggregate_read.sh
#
...
ITERS="${ITERS:-3}"
REPEATS="${REPEATS:-1}"   # multiply the shard.index N times so each iter reads
                          # N× the raw shard size — extends measurement window
                          # past Slurm task-launch jitter
...
python - "$DATA_ROOT" "$TASK_ID" "$ARRAY_SIZE" "$SHARD_INDEX" "$REPEATS" <<'PY'
import os, sys
root, task_id, array_size, out, repeats = sys.argv[1], int(sys.argv[2]), int(sys.argv[3]), sys.argv[4], int(sys.argv[5])
...
with open(out, "w") as f:
    for _ in range(repeats):
        f.write("\n".join(paths) + "\n")
print(f"task {task_id}/{array_size}: {len(mine)} classes, {len(paths)} unique files × {repeats} repeats -> {out}")
PY
