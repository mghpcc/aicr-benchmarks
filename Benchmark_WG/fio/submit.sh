...
for nj in $NUMJOBS_SWEEP; do
    ...
    # cpus-per-task = numjobs (1 worker per core, no oversubscription).
    # RUNTIME=60 keeps the seq_read write-eviction chain at its design size;
    # TOTAL_PER_JOB=3G / RAND_SIZE_PER_JOB=4G keep cluster footprints above
    # CBOX cache at BOTH sweep points (seq ≥12 TiB, rand ≥16 TiB). --time=02:00
    # absorbs the slow 1M file-create phase at these sizes. All three preserve
    # cache-defeat (see feedback_cache_defeat_invariant memory + note.md).
    b200_id=$(ARRAY_SIZE=21 TAG=$TAG WORKLOAD=$WORKLOAD SIZES="$SIZES" \
        ...
        IODEPTH=64 RAND_IODEPTH=128 RUNTIME=60 RAMP_TIME=5 \
        TOTAL_PER_JOB=3G RAND_SIZE_PER_JOB=4G \
            sbatch "${dep_args[@]}" \
                   -p b200-batch -J fio_peak_b200_n${nj} \
                   -N 2 --ntasks-per-node=1 --cpus-per-task=${nj} \
                   --time=02:00:00 --gres=gpu:0 \
